import { normalizeErrorTree } from "./normalizeErrorTree.mjs";

import type { NormalizedError } from "../index.mjs";
...
function generateTreeStructure(errors: Array<NormalizedError>, indent: string): string
{
	...
	errors.forEach(
		(error: NormalizedError, index: number): void =>
		{
			...
			result += `${indent}${CONNECTOR}${error.message}\n`;
			result += generateTreeStructure(error.causes, `${indent}${INDENT}`);
		}
	);
