...
describe("load", (): void => {
	it("should ignore non-mocked modules", async (): Promise<void> => {
	...
	it("should short-circuit mocked modules", async (): Promise<void> => {
	...
	it("should throw when it cannot find a module", async (): Promise<void> => {
		const TRAP_LOAD_RESULT: LoadResult = {
			shortCircuit: false,
			format: ModuleFormat.COMMON_JS,
			source: "test source",
