...
describe("convertSource", (): void => {
	it("should prepend an import of the MockStorage module", (): void => {
		...
		match(RESULT, /\bimport \{ MockStorage \} from ".*\/mock-storage\.mjs";/);
	});

	it("should replace mocked dependencies.", (): void => {
	...
	it("should ignore non-mocked dependencies.", (): void => {
		const SOURCE: string = `
			import { randomUUID } from "node:crypto";
	...
	it("should replace non-mocked dependencies relative path to the corresponding absolute path.", (): void => {
		const SOURCE: string = `
	...
	it("should append code that clear the storage from the mocks", (): void => {
		const SOURCE: string = "";
